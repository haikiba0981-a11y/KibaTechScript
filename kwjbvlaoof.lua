...
ToggleButton.Position = UDim2.new(0.4, 0, 0.78, 0)
ToggleButton.BackgroundColor3 = Color3.fromRGB(25, 25, 25)
ToggleButton.Text = "Lethal Kiba"
...
Gradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(80, 25, 120)),
...
CooldownFrame.Size = UDim2.new(0, 160, 0, 15)
CooldownFrame.Position = UDim2.new(0.5, -100, 0.70, 0)
CooldownFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
CooldownFrame.BorderSizePixel = 3
CooldownFrame.BorderColor3 = Color3.fromRGB(255, 0, 200) -- Pixel outline màu neon
...
BackPixel.BackgroundColor3 = Color3.fromRGB(40, 0, 60)
...
CooldownLabel.Font = Enum.Font.Arcade -- FONT PIXEL
...
local function findNearestPlayer()
    ...
    for _, descendant in pairs(workspace:GetDescendants()) do
        if descendant:IsA("Model") and descendant:FindFirstChild("HumanoidRootPart") and descendant ~= Character then
            local success, distance = pcall(function()
                return (RootPart.Position - descendant.HumanoidRootPart.Position).Magnitude
            end)
            if success and distance <= closestDistance then
...
local function performDash(target)
    ...
    teleportConnection = RunService.Heartbeat:Connect(function()
        if teleportDuration <= tick() - startTime then
    ...
    repeat task.wait() until teleportDuration <= tick() - startTime
...
local function activateAbility()
    ...
    if not targetHumanoid or targetHumanoid.Health / targetHumanoid.MaxHealth <= 0.15 then
        return -- check lúc phát hiện target
    end
...
local function onAnimationPlayed(animationTrack)
    ...
    local targetHumanoid = target:FindFirstChild("Humanoid")
    if not targetHumanoid or targetHumanoid.Health / targetHumanoid.MaxHealth <= 0.2 then
        return
    end
